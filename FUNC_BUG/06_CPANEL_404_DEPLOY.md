...
- Sau khi `git pull` hoặc khôi phục mã nguồn Laravel lên hosting cPanel sử dụng máy chủ web **LiteSpeed Web Server (LSWS)**:
  - Khi truy cập vào tên miền chính (`https://vattunongnghiep58.com/` hoặc `https://3nong.vn/`) hoặc bất kỳ tệp tin tĩnh nào (kể cả tệp `test.txt` đặt trong thư mục `public/` hay thư mục gốc), trình duyệt đều trả về trang báo lỗi mặc định của LiteSpeed:
  ```http
  404 Not Found
...
chmod 755 /home/username/domain.com
```
