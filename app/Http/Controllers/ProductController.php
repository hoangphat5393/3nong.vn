...
class ProductController extends Controller
{
    ...
    {
        // All category
        $categories = Category::where(['status' => 1, 'parent' => 0])
            ->orderBy('sort', 'asc')
        ...
        // default data
        $this->data['categories'] = $this->transformHomeCategories($categories);
        ...
        try {
            return response($html)->header('Cache-Control', 'public, max-age=120, stale-while-revalidate=30');
        } catch (\Exception $e) {
            return $html;
        }
    }
    ...
    // Product detail
    public function productDetail($slug, $id = null)
    {
        $query = Product::query();
        if ($id) {
            $query->where('id', $id);
        ...
        $primaryCategory = Category::whereHas('products', function ($query) use ($product) {
            $query->where('products.id', $product->id);
        })->first();

        if ($primaryCategory) {
            $related_products = $primaryCategory->products()
                ->where('status', 1)
