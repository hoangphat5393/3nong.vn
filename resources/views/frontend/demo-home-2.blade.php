                                            ...
                                            <div class="price-block-opt2">
                                                @if ($v->price_type === 'price' && (float) $displayPrice > 0)
                                                    <div class="price-line-primary">
                                                        <span
                                                            class="sale-price-opt2">{{ number_format($displayPrice, 0, ',', '.') }}đ{!! !empty($unitClean) ? ' <span class="sale-price-unit">/' . $unitClean . '</span>' : '' !!}</span>
                                                        @if ($hasPromo)
                                ...
                                <div class="swiper-wrapper">
                                    @foreach ($cat->products as $v1)
                                        @php
                                            ...
                                            $hasPromo1 =
                                                $salePrice1 > 0 && $regularPrice1 > 0 && $salePrice1 < $regularPrice1;
                                            $displayPrice1 = $hasPromo1 ? $salePrice1 : $regularPrice1;
                                            $discountPercent1 =
                                                $hasPromo1 && $regularPrice1 > 0
                                                    ? (int) round(
                                                        (($regularPrice1 - $salePrice1) / $regularPrice1) * 100,
                                                    )
                                                    : 0;
                                            $unitClean1 =
                                                !empty($v1->unit) &&
                                                ...
                                                <div class="product-list-item">
                                                    ...
                                                    <div class="price-block-opt2">
                                                        @if ($v1->price_type === 'price' && (float) $displayPrice1 > 0)
                                                            <div class="price-line-primary">
                                                                <span
                                                                    class="sale-price-opt2">{{ number_format($displayPrice1, 0, ',', '.') }}đ{!! !empty($unitClean1) ? ' <span class="sale-price-unit">/' . $unitClean1 . '</span>' : '' !!}</span>
                                                                @if ($hasPromo1)
                                                                    <span
                                                                        class="badge-inline-sale">-{{ $discountPercent1 }}%</span>
                                                                @endif
                                                            </div>
                                                            @if ($hasPromo1)
                                                                <div class="old-price-inline">
